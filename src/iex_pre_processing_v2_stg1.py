...
iex_output_folder = 'C:\\uday\\ml_nlp_dsci\\datasets\\iex_finance\\day_trade_lstm\\output_v3\\stage1\\'
window_size = 20
shift = 4
ma1_size = 4
ma2_size = 8
...
def create_intermediate_data(ticker, date, market_averages, market_volumes):
    ...
    for i in range(0, len(market_averages) - window_size - target_position + 1, shift):
        intermediate_data.append(
            [ticker, date] + market_averages[i:i + window_size + target_position] + market_volumes[i:i + window_size])
...
def process_iex_file(iex_file):
    ...
    stock_prices_daily_df = stock_prices_df.groupby('date')['label', 'marketAverage', 'marketVolume']
